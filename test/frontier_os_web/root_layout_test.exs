defmodule FrontierOSWeb.RootLayoutTest do
  @moduledoc """
  ...
  test "root layout renders page title with FrontierOS suffix" do
    html = render_root_layout("Mission Control")

    assert html =~ ~r/Mission Control\s*· FrontierOS/
    refute html =~ ~r/Mission Control\s*-\s*FrontierOS/
  end
  ...
  defp render_root_layout(page_title) do
    render_component(&FrontierOSWeb.Layouts.root/1,
