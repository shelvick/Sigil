defmodule FrontierOS.TribesTest do
  @moduledoc """
  ...
  alias FrontierOS.{Accounts, Assemblies, Cache, Tribes}
  ...
  describe "discover_members/2" do
    test "discovers tribe members from chain by tribe_id", %{
      ...
      assert is_struct(tribe, FrontierOS.Tribes.Tribe)
      assert Enum.map(tribe.members, & &1.character_id) == ["0xcharacter", "0xcharacter-3"]
      verify!()
    end

    test "marks members as connected when registered on FrontierOS", %{
      tables: tables,
      pubsub: pubsub,
      character_type: character_type
    } do
  ...
  test "full tribe flow - register, discover, list, assemblies", %{
    ...
    expect(FrontierOS.Sui.ClientMock, :get_objects, 3, fn filters, [] ->
      case {Keyword.get(filters, :type), Keyword.get(filters, :owner)} do
        {^character_type, ^address} ->
          {:ok, character_page([connected_character])}
  ...
  defp tribe_struct(overrides) do
    Map.merge(
      %{
        __struct__: FrontierOS.Tribes.Tribe,
        tribe_id: @tribe_id,
        members: [],
  ...
  defp tribe_member(overrides \\ %{}) do
    Map.merge(
      %{
        __struct__: FrontierOS.Tribes.TribeMember,
