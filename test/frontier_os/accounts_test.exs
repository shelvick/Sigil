defmodule FrontierOS.AccountsTest do
  @moduledoc """
  ...
  alias FrontierOS.{Accounts, Cache}
  ...
  describe "register_wallet/2" do
    test "register_wallet/2 caches account in ETS", %{
      ...
      expect(FrontierOS.Sui.ClientMock, :get_objects, fn [type: ^character_type, owner: ^address],
                                                         [] ->
        {:ok, page}
      end)

      assert {:ok, account} = Accounts.register_wallet(address, tables: tables, pubsub: pubsub)
      assert is_struct(account, FrontierOS.Accounts.Account)
    ...
    test "register_wallet/2 rejects invalid address format", %{tables: tables, pubsub: pubsub} do
      expect(FrontierOS.Sui.ClientMock, :get_objects, 0, fn _filters, _opts ->
        {:ok, character_page([])}
      end)
  ...
  describe "sync_from_chain/2" do
    test "sync_from_chain/2 updates cached account", %{
      ...
      expect(FrontierOS.Sui.ClientMock, :get_objects, 2, fn [
                                                              type: ^character_type,
                                                              owner: ^address
                                                            ],
                                                            opts ->
