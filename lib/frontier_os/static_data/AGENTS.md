# lib/frontier_os/static_data/

## Modules

- `FrontierOS.StaticData` — DETS-backed GenServer loading static data into process-owned ETS tables (`static_data.ex` in parent dir)
- `FrontierOS.StaticData.DetsFile` — Bounded atom pool DETS file operations (open, write, path resolution)
- `FrontierOS.StaticData.SolarSystem` — Struct + `from_json/1` for World API solar systems
- `FrontierOS.StaticData.ItemType` — Struct + `from_json/1` for World API item types
- `FrontierOS.StaticData.Constellation` — Struct + `from_json/1` for World API constellations
- `FrontierOS.StaticData.WorldClient` — Behaviour contract (3 callbacks: fetch_types, fetch_solar_systems, fetch_constellations)
- `FrontierOS.StaticData.WorldClient.HTTP` — Req-backed paginated HTTP implementation with retry
...
- WorldClient behaviour + Hammox mock for DI (`config :frontier_os, :world_client`)
