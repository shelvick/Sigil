...
## Modules
- `SigilWeb.Router` (`router.ex`) — wallet-session LiveView routes including dashboard, diplomacy, tribe intel, alerts, marketplace, and galaxy map.
...
- `SigilWeb.GalaxyMapLive` (`live/galaxy_map_live.ex`) — interactive galaxy map with Three.js point-cloud, intel/marketplace overlays, system selection, and PubSub-driven real-time updates.
  - `.Components` (`live/galaxy_map_live/components.ex`) — map canvas panel + detail panel HEEx components.
  - `.Data` (`live/galaxy_map_live/data.ex`) — detail-panel data builder + system/constellation payload mappers.
- `SigilWeb.IntelLive`, `SigilWeb.TribeOverviewLive`, and `SigilWeb.AssemblyDetailLive` — tribe intel views that share data with the marketplace seller workflow. Assembly detail, intel feed, and marketplace listing cards include "View on Map" links.
...
- `assets/js/hooks/galaxy_map.js` — Three.js point-cloud galaxy renderer with orbit controls, constellation clustering, click-to-select, and overlay layers. Event bridge to LiveView via handleEvent/pushEvent.
- `assets/js/hooks/galaxy_map_utils.js` — pure utility functions for coordinate normalization, system index, overlay position resolution, camera setup, constellation threshold.
