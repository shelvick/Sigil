defmodule Sigil.Diplomacy do
  ...
  @spec discover_custodian(non_neg_integer(), options()) ::
          {:ok, custodian_info() | nil} | {:error, Client.error_reason()}
  def discover_custodian(tribe_id, opts)
    ...
    with {:ok, %{data: objects}} <- client.get_objects([type: custodian_type()], req_options) do
      ...
      if custodian do
        set_active_custodian(custodian, Keyword.put(opts, :tribe_id, tribe_id))
      end
  ...
  def get_standing(target_tribe_id, opts) when is_integer(target_tribe_id) and is_list(opts) do
    source_tribe_id = source_tribe_id(opts)

    case Cache.get(standings_table(opts), {:tribe_standing, source_tribe_id, target_tribe_id}) do
      nil -> :neutral
  ...
  def list_standings(opts) when is_list(opts) do
    ...
    |> Enum.map(fn {{:tribe_standing, ^source_tribe_id, target_tribe_id}, value} ->
  ...
  def list_pilot_standings(opts) when is_list(opts) do
    ...
    standings_table(opts)
    |> Cache.match({{:pilot_standing, source_tribe_id, :_}, :_})
    |> Enum.map(fn {{:pilot_standing, ^source_tribe_id, pilot}, value} ->
  ...
  def get_pilot_standing(pilot, opts) when is_binary(pilot) and is_list(opts) do
    ...
    case Cache.get(standings_table(opts), {:pilot_standing, source_tribe_id, pilot}) do
      nil -> :neutral
      value -> ObjectCodec.standing_to_atom(value)
    end
  end
  ...
  def get_default_standing(opts) when is_list(opts) do
    ...
    case Cache.get(standings_table(opts), {:default_standing, source_tribe_id}) do
      nil -> :neutral
      value -> ObjectCodec.standing_to_atom(value)
    end
  end
  ...
  defp active_tribe_id(custodian, opts) do
